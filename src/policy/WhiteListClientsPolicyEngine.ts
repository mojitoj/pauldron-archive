...
export class WhiteListClientsPolicyEnginePolicy extends Policy {
    content: {
        ...
        defaultRedirectAuthServer: string;
    };
}
    ...
    public evaluate(claims: Claims, policy: WhiteListClientsPolicyEnginePolicy): PolicyDecision {
        ...
        const redirectDecision: PolicyDecision = {
            ...
            obligations: [{
                    ...
                    params: {"uma_server": policy.content.defaultRedirectAuthServer}
                }]
        };

        if (policy.content.deniedClients && this.matches(claims, policy.content.deniedClients)) {
            return denyDecision;
        } else if (policy.content.permittedClients && this.matches(claims, policy.content.permittedClients)) {
